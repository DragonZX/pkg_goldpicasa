...
class modDJImageSliderHelper
{
	...
	static function getNavigation(&$params, &$mid) {
		...
		if($params->get('slider_type')==1) {			
			if(empty($prev) || !file_exists(JPATH_ROOT.DS.$prev)) $prev = 'modules/mod_goldslider/themes/'.$theme.'/images/up.png';
			if(empty($next) || !file_exists(JPATH_ROOT.DS.$next)) $next = 'modules/mod_goldslider/themes/'.$theme.'/images/down.png';
		} else {			
			if(empty($prev) || !file_exists(JPATH_ROOT.DS.$prev)) $prev = 'modules/mod_goldslider/themes/'.$theme.'/images/prev.png';
			if(empty($next) || !file_exists(JPATH_ROOT.DS.$next)) $next = 'modules/mod_goldslider/themes/'.$theme.'/images/next.png';
		}
		if(empty($play) || !file_exists(JPATH_ROOT.DS.$play)) $play = 'modules/mod_goldslider/themes/'.$theme.'/images/play.png';
		if(empty($pause) || !file_exists(JPATH_ROOT.DS.$pause)) $pause = 'modules/mod_goldslider/themes/'.$theme.'/images/pause.png';
